<template>
   ...
   <div class="container">
    <h2 class="center deep-purple-text">SMS Operation Platform</h2>
      ...
      <router-link :to="{ name: 'HomeOp',params:{ shift: this.shift,date: this.date,truckNum: this.truckNum,month: this.month, year: this.year,username: this.username, docidLoadSuper : this.docidLoadSuper}}">
             ...
             <div class="section">
               <h5 class=" col s12 left deep-purple-text" style="margin-bottom: 20px;">Select Reduced Level</h5>
             <div class="row" >

            <div class="col s3" v-for="(rl,index) in RLs" :key="index">
              <!-- <router-link :to="{ name: 'HomeOp',params:{ docidDCOp : docidDCOp, docidLoadSuper: docidLoadSuper}}"> -->
                  <i class="waves-effect waves-light btn-large " @click="addLoad(rl)">{{rl}}</i>
                  <br>
                  <br>
              <!-- </router-link> -->
            </div>
        ...
        <div class="divider"></div>
          ...
          <div class="left field">
            <router-link :to="{ name: 'DigUnit',params:{ PreStartID : this.PreStartID,shift: this.shift,date: this.date,truckNum: this.truckNum,month: this.month, year: this.year,username: this.username,docidLoadSuper : docidLoadSuper,LoadId: LoadId,from:'RL'}}">
           ...
           <div class="field center">
            <i class="deep-purple btn-small" @click="delLoad" >Cancel load </i>
          </div>
...
export default {
   ...
   addLoad(newitem){
            ...
            refLR.get().then( docsnapshot => {
                  ...
                  .then(() => {
                    // go to next action
                      this.$router.push({ name: 'shot', 
                          params:{ PreStartID : this.PreStartID,LoadId : this.LoadId, shift: this.shift,date: this.date,truckNum: this.truckNum,month: this.month, year: this.year,username: this.username, docidLoadSuper : this.docidLoadSuper}})
                ...
                else
                {
                 ...
                 this.feedback = "this load record does not exist,Please go back to create one"
  ...
  //        refLR.onSnapshot(snapshot => {

  //         this.load.push(snapshot.data().Activity)
  //         this.load.push(snapshot.data().DigUnit)        
  //   })

  let refRL = db.collection('operator-DC').doc(this.LoadId)

    refRL.get().then( docsnapshot => {
