  ...
  <div class="container">
    <h2 class="center deep-purple-text">SMS Operation Platform</h2>
        ...
        <div class="divider"></div>
          <div class="section">
            <h5 class=" col s12 left deep-purple-text" style="margin-bottom: 20px;">Select Material</h5>
        <div class="row" >
          <div class="col s3" v-for="(Material,index) in Materials" :key="index">
              <!-- <router-link :to="{ name: 'HomeOp',params:{ docidDCOp : docidDCOp, docidLoadSuper: docidLoadSuper}}"> -->
                  <i class="waves-effect waves-light btn-large " @click="addLoad(Material)">{{Material}}</i>
                  <br>
                  <br>
              <!-- </router-link> -->
            </div>
         ...
         <p style=" color: red; ">Current Path:</p>
          ...
          <p>Activity -->Dig Unit -->Shot -->Material --> </p>
        ...
        <div class="divider"></div>
          ...
          <div class="left field">
            <router-link :to="{ name: 'shot',params:{ PreStartID : this.PreStartID,shift: this.shift,date: this.date,truckNum: this.truckNum,month: this.month, year: this.year,username: this.username,docidLoadSuper : docidLoadSuper,LoadId: LoadId,from:'RL'}}">
      ...
      let ref = db.collection('load-haul-surper').doc(this.docidLoadSuper)
        console.log(ref);
         ref.onSnapshot(snapshot => {
          this.Materials = snapshot.data().material 
          console.log(this.Materials);  
    })

    let refMaterial = db.collection('operator-DC').doc(this.LoadId)

    refMaterial.get().then( docsnapshot => {
       ...
       // get the current load record we dont get the info in realtime
  // let refLR = db.collection('Load-op').doc(this.LoadId)
        
  //        refLR.onSnapshot(snapshot => {

  //         this.path.push(snapshot.data().Activity)
  //         this.path.push(snapshot.data().DigUnit)        
  //         this.path.push(snapshot.data().RL) 
  //         this.path.push(snapshot.data().Shot) 
  //   })
