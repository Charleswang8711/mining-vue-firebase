...
<div class="container">
    <h2 class="center deep-purple-text">SMS Operation Platform</h2>
             ...
             <div class="section">
               <h5 class=" col s12 left deep-purple-text" style="margin-bottom: 20px;">Select Shot</h5>
             <div class="row" >

            <div class="col s3" v-for="(shot,index) in shots" :key="index">
              <!-- <router-link :to="{ name: 'HomeOp',params:{ docidDCOp : docidDCOp, docidLoadSuper: docidLoadSuper}}"> -->
                  <i class="waves-effect waves-light btn-large " @click="addLoad(shot)">{{shot}}</i>
                  <br>
                  <br>
              <!-- </router-link> -->
            </div>
        ...
        <div class="divider"></div>
          ...
          <div class="left field">
            <router-link :to="{ name: 'RL',params:{PreStartID : this.PreStartID,shift: this.shift,date: this.date,truckNum: this.truckNum,month: this.month, year: this.year,username: this.username,docidLoadSuper : docidLoadSuper,LoadId: LoadId}}">
      ...
      let ref = db.collection('load-haul-surper').doc(this.docidLoadSuper)
        console.log(ref);
         ref.onSnapshot(snapshot => {
          this.shots = snapshot.data().shot 
          console.log(this.shots);  
    })

  //      // get the current load record
  // let refshot = db.collection('Load-op').doc(this.LoadId)
        
  //        refshot.onSnapshot(snapshot => {

  //         this.load.push(snapshot.data().Activity)
  //         this.load.push(snapshot.data().DigUnit)  
  //         this.load.push(snapshot.data().RL)      
  //   })

     let refshot = db.collection('operator-DC').doc(this.LoadId)

    refshot.get().then( docsnapshot => {
